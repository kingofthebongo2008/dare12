...
namespace cuda
{
    ...
    __global__ void kernel_gray_scale( const uint8_t* rgb_t, uint8_t* grayscale, uint32_t width, uint32_t height, uint32_t row_pitch_src, uint32_t row_pitch_dst )
    {
        ...
        if (x < width && y < height)
        {
            const rgb* rgb_ = reinterpret_cast<const rgb*> (rgb_t +  y * row_pitch_src + x * sizeof ( rgb ) ) ;
            ...
            grayscale[y * row_pitch_dst + x] = gray_quantized;
        }
    }

    std::tuple < dim3, dim3 > create_texture_kernel_params( uint32_t width, uint32_t height )
    {
        //1x1 squares
    ...
    imaging::cuda_texture create_grayscale_texture ( const imaging::cuda_texture& texture_color )
    {
        ...
        kernel_gray_scale << < std::get<0>(params), std::get<1>(params) >> >  (texture_color.get_gpu_pixels(), t.get_gpu_pixels(), width, height, texture_color.get_pitch(), t.get_pitch() );
