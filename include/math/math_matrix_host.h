...
namespace math
{
...
#if defined(WIN32)
    #define float4x4_param const float4x4&
#else
    typedef float4x4_param float4x4;
#endif
    ...
    inline void store44(float* __restrict address, float4x4_param m)
	{
    ...
    inline void store43(float* __restrict address, float4x4_param m)
	{
		store4(address, m.r[0]);
		store4(address+4, m.r[1]);
		store4(address+8, m.r[2]);
    ...
    inline float4x4 transpose(float4x4_param m)
	{
		float4x4 m1;
    ...
    inline float4x4 inverse(float4x4_param m)
	{
		float4 tmp;
		float4 det;
		float4 minor0; 
    ...
    inline float4 mul(float4 v, float4x4_param m)
	{
    ...
    inline float4 mul(float4x4_param m, float4 v)
	{
		float4x4 trans = transpose(m);
		return mul(v, trans);
	}

    inline float4x4 mul(float4x4_param m1, float4x4_param m2)
	{
		float4x4 m;
    ...
    inline float4x4 add(float4x4_param m1, float4x4_param m2)
	{
		float4x4 m;
    ...
    inline float4x4 matrix44_sub(float4x4_param m1, float4x4_param m2)
	{
		float4x4 m;
    ...
    inline float4x4 matrix44_mad(float4x4_param m1, float4x4_param m2, float4x4_param m3)
	{
